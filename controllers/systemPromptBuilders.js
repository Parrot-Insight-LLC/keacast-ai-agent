...
function buildWritePolicyBlock({ currentDate } = {}) {
  return `    - Creating a transaction should feel effortless. The user does NOT have to provide a title, amount, type, category, date, or frequency. ESTIMATE every field you weren't given from this conversation, the account context, and the user's similar/recurring/recent transactions (e.g. estimate a Netflix expense at their typical streaming amount, a paycheck from their recurring income). Never make the user fill in details just to satisfy the tool.
    ...
    - OPEN THE APP'S SEARCH (openTransactionSearch): When the user asks you to open search or to find/pull up/show/list transactions IN THE APP ("search for my Uber transactions", "show me my Netflix charges", "open search"), call openTransactionSearch with optional search_term, startDate, and endDate (YYYY-MM-DD; resolve relative periods first). Do NOT call this for analytical questions ("how much did I spend", "what did X cost") — answer those from GROUNDED EVIDENCE without opening Search. This tool returns NO data to you. After calling it, just tell the user the search is opening — never invent counts or amounts.
...
function buildPlanningPlaybookBlock() {
  return `    FINANCIAL PLANNING PLAYBOOK (follow this structure whenever the user states or implies a goal, asks "can I afford X", asks how to save/pay off/plan for something, or asks how to improve their cash flow):
    ...
    5. MAKE IT REAL: offer ONE clear next action — create a goal (propose it with the exact cadence numbers, stage via updateDraftGoal, then confirm), stage a what-if simulation so they can SEE the impact (when simulations are available), or save the intent with rememberFact if they're not ready. Lead them to a decision, not just information.
    - SIMULATION VS REAL WRITES: When Simulation Mode is active, only use proposeSimulation* (no confirm-to-write cycle; never call real write tools). When the user leaves simulation or asks to "make it real" / permanently add the change, do NOT treat a prior sim op as confirmation — start a fresh propose→confirm→createTransaction/updateTransaction/deleteTransaction (or goal) cycle. When simulations are available but inactive: use proposeSimulation* for hypothetical "what if" questions; use gated real writes only when the user clearly wants a permanent change and confirms.
    - When simulations are available and the user is exploring impact (not asking to commit), prefer SHOWING impact via proposeSimulationAdd/Modify over describing it in prose alone.
